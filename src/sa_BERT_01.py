...
databaseName2Info = {'imdb': {'dir': "IMDB Reviews", 'fnc': data4SAandBERT.readImdbData},
                     'rt': {'dir': "RT_Sentiment", 'fnc': data4SAandBERT.readRTData}
                     }
...
def getExamples(df, dataInfo, guid=None, text_b=None):
    ...
    if any(x for x in cols if 'pol' == x):
        ...
        dataInfo.LABEL_COLUMN = 'polarity'
    print(f'cols: {cols}')
...
def getData(DATA_INFO, DATA_LOCATION_RELATIVE_TO_CODE, DATA_VERSION_APPENDIX, currentTrainDataName, currentTestDataName, databaseName2Info,inputDir):
    trainDBInfo = databaseName2Info[currentTrainDataName]
    testDBInfo = databaseName2Info[currentTestDataName]
    trainDBDir = trainDBInfo['dir']
    testDBDir = testDBInfo['dir']
    trainDBFnc = trainDBInfo['fnc']
    testDBFnc = testDBInfo['fnc']
    # ******************************************************
    train_dir_Imdb = Path(DATA_LOCATION_RELATIVE_TO_CODE) / ('train' + DATA_VERSION_APPENDIX)
    # **************************************************************
    train_dir_Imdb = os._fspath(train_dir_Imdb)
    print(f'Preparing to read from: {train_dir_Imdb}')
    imdbTrainData = trainDBFnc(DATA_LOCATION_RELATIVE_TO_CODE, DATA_VERSION_APPENDIX, readFromSource=DATA_INFO.READ_FROM_SOURCE)
    # **************************************************************
    print('Creating examples from train data')
    # %%
    # test_dir_Imdb = 'Data/sentiment/Data/IMDB Reviews/IMDB Data/test/'
    ...
    test_dir_Imdb = train_dir_ImdbP = Path(DATA_LOCATION_RELATIVE_TO_CODE) / (
            'test' + DATA_VERSION_APPENDIX)  # 'Data/sentiment/Data/IMDB Reviews/IMDB Data/test_short/'
    imdbTestData = testDBFnc(DATA_LOCATION_RELATIVE_TO_CODE, DATA_VERSION_APPENDIX, readFromSource=DATA_INFO.READ_FROM_SOURCE)
    print('Creating examples from test data')
    return imdbTestData, imdbTrainData
...
def main():
    ...
    pdPerformance = pd.DataFrame(
        columns=['auc', 'eval_accuracy', 'f1_score', 'false_negatives', 'false_positives', 'loss', 'precision',
    ...
    for currentTrainDataName in data2Use2Train:
        ...
        for currentTestDataName in data2Use2Train:
            currentTestDBInfo = databaseName2Info[currentTestDataName]
            PROCESSING_INFO, evaluationResults, myCWD = getPerformanceMeassure(DATA_VERSION_APPENDIX, inputInfo2Save,
                                                                               currentTrainDataName, currentTestDataName,databaseName2Info)
...
def getPerformanceMeassure(DATA_VERSION_APPENDIX, inputInfo2Save, currentTrainDataName, currentTestDataName, databaseName2Info):
    currentTrainDBInfo=databaseName2Info[currentTrainDataName]
    currentTestDBInfo=databaseName2Info[currentTestDataName]
    currentTrainDBDir = currentTrainDBInfo['dir']
    currentTestDBDir = currentTestDBInfo['dir']
    currentTrainDBFnc = currentTrainDBInfo['fnc']
    currentTestDBFnc = currentTestDBInfo['fnc']
    dataTrainPath = str(Path(params.inputDirectoryIMDBData) / currentTrainDBDir)
    dataTestPath = str(Path(params.inputDirectoryIMDBData) / currentTestDBDir)
    inputInfo2Save['inputPath'] = params.inputDirectoryIMDBData
    print(f'Train path: {dataTrainPath}')
    print(f'Test path: {dataTrainPath}')
    myCWD = Path.cwd()
    print(f'cwd = {myCWD}')
    # imdbRelPath= Path(imdbDataPath).relative_to(myCWD)
    # imdbRelPath= Path(imdbDataPath).relative_to(myCWD)
    # print(PurePath(imdbDataPath).relative_to(myCWD))
    dataTrainRelPath = os.path.relpath(dataTrainPath, myCWD)
    dataTestRelPath = os.path.relpath(dataTestPath, myCWD)
    DATA_LOCATION_RELATIVE_TO_CODE = dataTrainRelPath
    print(f'Train data relative path  = {dataTrainRelPath}')
    ...
    imdbTestData, imdbTrainData = getData(DATA_INFO, DATA_LOCATION_RELATIVE_TO_CODE, DATA_VERSION_APPENDIX,
                                          currentTrainDataName, currentTestDataName, databaseName2Info,params.inputDirectoryIMDBData)
    # %% Process data
